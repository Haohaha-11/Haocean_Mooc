...
VERIFY_B_PORT="${VERIFY_B_PORT:-18080}"
BASE_URL="${BASE_URL:-http://127.0.0.1:${VERIFY_B_PORT}}"
...
MODULE_B_AUTH_REQUIRED=false MODULE_B_DEV_VERIFICATION_LOG=true \
.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port "${VERIFY_B_PORT}" > "${SERVER_LOG}" 2>&1 &
SERVER_PID=$!
...
BASE_URL="${BASE_URL}" bash scripts/smoke_test.sh
...
MODULE_A_ASSIGNMENT_ID="${ASSIGNMENT_ID}" \
MODULE_A_SERVER_URL="${BASE_URL}" \
