...
def _usage_lines(role_label: str) -> list[str]:
    if role_label == "Teacher":
        return [
            "Welcome Teacher !",
            "After login: pending review TUI opens.",
            "Enter review | Ctrl+S submit score.",
            "p pending | a approved | r refresh | q quit.",
        ]
    return [
        "Welcome Student !",
        "After login: join class, list assignments.",
        "Submits ~/.haocean/workspace/<assignment_id>/.",
        "Run feedback to save ~/.haocean/feedback_inbox/.",
    ]
...
def ensure_student_auth(
    ...
    *,
    ...
    student_id = settings.student_id.strip() or input("Student ID : ").strip()
    if not email:
        raise SystemExit("MODULE_A_EMAIL or interactive email is required")
    if not student_id:
        raise SystemExit("MODULE_A_STUDENT_ID or STUDENT_ID is required")

    payload = client.request_login_code(email=email, student_id=student_id)
    ...
    login_payload = client.login_with_code(email=email, student_id=student_id, code=code)
